...
class TextSearcher:
    def __init__(self, dataset_path: str, law_path:str, queries_path: str, rows: int=None):
        ...
        """
        self._columns = ['itemid','appno', 'docname', 'doctype', 'article', 'violation', 'year', 'THE_LAW']
        # read the full dataset but with less columns to save loading time
        self.full_dataset = pd.read_csv(dataset_path, usecols=self._columns[:-1], nrows=rows, low_memory=False)
        self.law = pd.read_csv(law_path) 
        ...
        tmp_df = self.full_dataset.merge(self.law, how='left', on='itemid')
        # Filter out cases where we are missing the full dataset
        self.dataset = tmp_df.loc[~pd.isnull(tmp_df['THE_LAW'])]
    ...
    def search_text(self, query_word: str, language_filter: str = None, context_words: int = 10) -> List[Dict]:
        ...
        """
        results = []
        language_column = 'languageisocode'
        
        # Filter by language if specified
        search_data = self.dataset.copy()
        if language_filter:
            search_data = search_data[search_data[language_column].str.lower() == language_filter.lower()]
